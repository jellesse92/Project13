...
public class BulletSourceScript : MonoBehaviour {
    ...
    const float STUN_MULTI = 1f;                    //Multiplier for how long enemies should be stunned after hit
    ...
    public void QuickShot(int damage)
    {
        ...
        if (hit.collider != null)
        {
            ...
            {
                ApplyQuickDamage(hit.collider.gameObject, damage);
            }
    ...
    void ApplyQuickDamage(GameObject target, int damage)
    {
        if(target.tag == "Enemy")
        {
            target.GetComponent<Enemy>().Damage(damage, STUN_MULTI);
